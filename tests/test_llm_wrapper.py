...
@pytest.mark.asyncio
async def test_acomplete_stream_yields_chunks_and_caches(wrapper: LLMWrapper) -> None:
    ...
    with patch.object(wrapper.router, "acompletion", return_value=fake_chunks()):
        stream_meta: dict = {}
        emitted = [
            piece
    ...
    assert stream_meta["cache_hit"] is False
    assert stream_meta["usage"]["input_tokens"] == 9
    assert stream_meta["usage"]["output_tokens"] == 3

    stream_meta_cached: dict = {}
    with patch.object(wrapper.router, "acompletion") as router_call:
        replayed = [
            piece
            async for piece in wrapper.acomplete_stream(
                system_prompt="sys",
                ...
                max_tokens=4000,
                temperature=0.3,
                stream_meta=stream_meta_cached,
            )
        ]
    assert router_call.call_count == 0
    assert "".join(replayed) == "Hello world"
    assert stream_meta_cached["cache_hit"] is True
    assert stream_meta_cached["usage"]["input_tokens"] == 9
